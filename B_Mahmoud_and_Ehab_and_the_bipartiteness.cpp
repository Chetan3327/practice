...
void dfs(ll node, ll par, vll &colors, ll col, const vvll& adj){
  colors[node] = col;
  for(auto itr: adj[node]){
    if(itr != par){
      dfs(itr, node, colors, !col, adj);
    }
  }
}
void solve(){
  ...
  vll colors(n + 1, -1);
  dfs(1, -1, colors, 0, adj);

  ll ones = count(all(colors), 1);
  ll zeros = count(all(colors), 0);

  print(zeros * ones - (n - 1));
}
int main(){
  fastio
  solve();
