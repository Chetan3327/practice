...
void solve(){
  ...
  ll size = accumulate(all(freq), 0LL);

  // dp[idx][count] = dp[idx + 1][count - freq[idx]] + dp[idx + 1][count];
  ll count = (size + 1) / 2;
  

  ll ans = 1LL;
  ans = mod_mul(ans, fact[size / 2], MOD);
  ans = mod_mul(ans, fact[(size + 1) / 2], MOD);
  for(int i = 0; i < 26; i++){
    ans = mod_mul(ans, inv_fact[freq[i]], MOD);
  }
  print(ans);
}
int main(){
