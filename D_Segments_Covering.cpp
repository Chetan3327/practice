...
vll dp;
ll solve(ll idx, ll m, vector<vector<vector<ll>>> &a){
  if(idx > m) return 1;
  ...
  for(auto itr: a[idx]){
    ...
    curr = mod_mul(curr, p, MOD);
    curr = mod_div(curr, q - p, MOD);

    ans = mod_add(ans, mod_mul(curr, solve(r + 1, m, a), MOD), MOD);
  }
...
int main(){
  ...
  for(int i = 0; i < n; i++){
    ...
    notAllP = mod_mul(notAllP, q - p, MOD);
    notAllP = mod_div(notAllP, q, MOD);
  }

  dp.resize(m + 2, -1);

  ll ans = mod_mul(solve(1, m, a), notAllP, MOD);
  print(ans);  
}