...
void solve(){
  ...
  vll first = gen(0, n / 2 - 1);
  vll second = gen(n / 2, n - 1);
  ...
  for(auto itr: first){
    ...
    auto litr = lower_bound(second.begin(), second.end(), rem);
    auto ritr = upper_bound(second.begin(), second.end(), rem);
    ans += ritr - litr;
  }
  print(ans);
}
