...
struct segmenttree {
  ...
  ll query(ll s, ll e, ll x, ll l, ll r, ll node){
    if(s > r || e < l) return -1;
    if(a[node] < x) return -1;

    if(s == e) return s;

    ll mid = s + (e - s) / 2;

    if(a[2 * node + 1] >= x && l <= mid){
      ll leftans = query(s, mid, x, l, r, 2 * node + 1);
      if(leftans != -1) return leftans;
    }

    return query(mid + 1, e, x, l, r, 2 * node + 2);
  }
  ll query(ll x, ll l, ll r){
    return query(0, n - 1, x, l, r, 0);
  }
