...
void solve(){
  ...
  map<ll, ll> freq;
  map<ll, multiset<ll>> levels;

  ll maxfreq = 0;
  for(int i = 0; i < n; i++){
    // include a[i];

    ll currfreq = freq[a[i]];
    levels[currfreq].erase(a[i]);

    freq[a[i]]++;
    levels[currfreq + 1].insert(a[i]);

    maxfreq = max(maxfreq, currfreq + 1);

    // remove a[i - k];
    if(i >= k){
      ll currfreq = freq[a[i - k]];
      levels[currfreq].erase(a[i - k]);
      if(levels[currfreq].empty()){
        if(currfreq == maxfreq) maxfreq--;
      }

      freq[a[i - k]]--;
      levels[currfreq - 1].insert(a[i - k]);
    }

    if(i >= k - 1){
      ll ans = *levels[maxfreq].begin();
      cout << ans << sp;
    } 
  }
}
