...
// k -> total moves, z -> left allowed
ll dp[100005][6];
ll dfs(ll idx, const vll &a, ll n, ll k, ll z){
  if(k == 0) return a[idx];
  ...
  if(idx + 1 < n){
    ans = max(ans, dfs(idx + 1, a, n, k - 1, z));
  }

  if(idx > 0 && z > 0){
    ans = max(ans, dfs(idx - 1, a, n, k - 1, z - 1));
  }

  return dp[idx][z] = a[idx] + ans;
}
void solve(){
  ll n, k, z;
  cin >> n >> k >> z;
