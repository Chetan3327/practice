...
bool dfs(ll node, ll par, vll &vis, const vector<set<ll>> &adj){
  vis[node] = 1;

  for(auto itr: adj[node]){
    if(!vis[itr]){
      if(dfs(itr, node, vis, adj)) return true;
    }else if(itr != par){
      return true;
    }
  }
...
void solve(){
  ...
  for(int i = 0; i < n; i++){
    if(!vis[i]){
      connectedcomponents++;
      if(dfs(i, -1, vis, adj)) cycles++;
    }
  }

  ll minimum = cycles + (connectedcomponents > cycles);
  ll maximum = connectedcomponents;

  cout << minimum << sp << maximum << endl;
}
int main(){
  fastio
  testcases{
