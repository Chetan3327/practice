#include<bits/stdc++.h>
using namespace std;
#define fastio ios_base::sync_with_stdio(false);cin.tie(nullptr);cout.tie(nullptr);
#define ll long long
#define ld long double
#define endl "\n"
#define sp " "
#define testcases ll ts; cin >> ts; for(ll tsno = 1; tsno <= ts; tsno++)
typedef vector<ll> vll;
typedef vector<vector<ll>> vvll;
#define all(x) (x).begin(), (x).end()
#define rall(x) (x).rbegin(), (x).rend()
template <typename T> void input(vector<T> &a){for(auto &e: a) cin >> e;}
template <typename T> void input(vector<vector<T>> &a){for(auto &r: a) for(auto &c: r) cin >> c;}
template <typename T> void print(T ans){cout<<ans<<"\n";}
template <typename T> void print(vector <T> &ans){for(auto &i : ans) cout<<i<<" "; cout<<"\n";}
template <typename T> void print(vector <vector<T>> &ans){for(auto &i : ans) {{for(auto &j: i) cout<<j<<" ";} cout<<"\n";}}
#define MOD 1000000007
ll floor(ll a, ll b) {ll c = a / b; if (a % b != 0 && ((a < 0 && b > 0) || (a > 0 && b < 0))) c--; return c;}
ll gcd(ll a, ll b) {if (b > a) {return gcd(b, a);} if (b == 0) {return a;} return gcd(b, a % b);}
ll mod_add(ll a, ll b, ll m){a = a % m; b = b % m; return (((a + b) % m) + m) % m;}
ll mod_mul(ll a, ll b, ll m){a = a % m; b = b % m; return (((a * b) % m) + m) % m;}
ll mod_sub(ll a, ll b, ll m){a = a % m; b = b % m; return (((a - b) % m) + m) % m;}
ll expo(ll a, ll b, ll mod) {ll res = 1; while (b > 0) {if (b & 1)res = (res * a) % mod; a = (a * a) % mod; b = b >> 1;} return res;}
ll mminvprime(ll a, ll b) {return expo(a, b - 2, b);}
ll mod_div(ll a, ll b, ll m) {a = a % m; b = b % m; return (mod_mul(a, mminvprime(b, m), m) + m) % m;}
ll ceil_div(ll a, ll b) {return a / b + ((a ^ b) > 0 && a % b != 0);}
vector<pair<ll, ll>> dirs = {{-1, 0}, {0, 1}, {1, 0}, {0, -1}};
vector<bool> prime;
vector<ll> primes;
vector<ll> smallestfactor;
void generatePrimes() {
    const ll N = 1e6;
    if (!prime.empty()) return;
    prime.resize(N + 1, 1);
    smallestfactor.resize(N + 1, 1);
    prime[0] = prime[1] = 0;
    for (ll i = 2; i <= N; i++) {
        if (prime[i]) {
            smallestfactor[i] = i;
            primes.push_back(i);
            for (ll j = 2 * i; j <= N; j += i) {
                if(prime[j]){
                  prime[j] = 0;
                  smallestfactor[j] = i;
                }
            }
        }
    }
}
vector<pair<ll, ll>> prime_factorization(ll n){
  vector<pair<ll, ll>> factors;
  while(n > 1){
    ll p = smallestfactor[n];
    ll c = 0;
    while(n % p == 0){
      n /= p;
      c++;
    }
    factors.push_back({p, c});
  }
  return factors;
}
const int N = 100000;
void solve(){
  ll n;
  cin >> n;

  vll a(n);
  input(a);
  
  vll dp(N + 1, 0);
  ll ans = 0;
  for(int i = 0; i < n; i++){
    ll x = a[i];

    vector<pair<ll, ll>> factors = prime_factorization(x);

    ll curr = 0;
    for(auto &f: factors){
      if(x % f.first == 0){
        curr = max(curr, dp[f.first]);
      }
    }

    curr++;

    for(auto &f: factors){
      if(x % f.first == 0){
        dp[f.first] = max(dp[f.first], curr);
      }
    }

    ans = max(ans, curr);
  }
  print(ans);
}
int main(){
  fastio
  generatePrimes();
  solve();
}