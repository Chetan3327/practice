...
void solve(){
  ...
  pair<ll, ll> source = v[a];
  pair<ll, ll> dest = v[b];
  ll dist = abs(source.first - dest.first) + abs(source.second - dest.second);

  ll fromsource = 1e18;
  ll fromdest = 1e18;
  for(int i = 0; i < k; i++){
    pair<ll, ll> curr = v[i];
    fromsource = min(fromsource, (abs(source.first - curr.first) + abs(source.second - curr.second))); 
    fromdest = min(fromdest, (abs(dest.first - curr.first) + abs(dest.second - curr.second))); 
  }
  print(min(dist, fromsource + fromdest));    
}
int main(){
  fastio
  testcases {
