...
void solve(){
  ll n, p, lessonpoints, taskpoints;
  cin >> n >> p >> lessonpoints >> taskpoints;

  ll l = 0;
  ll r = n;
  ll ans = 0;

  ll totaltasks = ceil_div(n, 7);
  while(l <= r){
    ll mid = l + (r - l) / 2;

    ll curr = mid * lessonpoints; // lessons
    curr += min(totaltasks, 2 * mid) * taskpoints; // tasks
