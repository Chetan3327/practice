...
void solve(){
  ...
  vvll idxs;
  for(auto itr: mp) idxs.push_back(itr.second);
  ...
  // idxs[0][0] will be connected to all nodes other than 0's
  // the idxs[1][0] will be connected to all 0 nodes except the first([0][0]) one;
  
  for(int i = 1; i < idxs.size(); i++){
    for(auto itr: idxs[i]){
      cout << idxs[0][0] + 1 << sp << itr + 1 << endl;
    }
  }

  for(int i = 1; i < idxs[0].size(); i++){
    cout << idxs[0][i] + 1 << sp << idxs[1][0] + 1 << endl;
  }
}
int main(){
  fastio
