...
void solve(){
  ...
  ll xblocks = 0;
  ll yblocks = 0;
  bool yblockofsizemorethan1 = false;
  ll smallestxblock = n;
  ...
  for(ll i = 0; i < n; ){
    if(s[i] == 'A'){
      ...
      xblocks++;
      smallestxblock = min(smallestxblock, c);
    }else{
      ll c = 0;
      while(i < n && s[i] == 'B'){
        i++;
  ...
  if(xblocks <= yblocks){
    ...
    if(yblockofsizemorethan1){
      cout << countofx << endl;
    }else{
      cout << countofx - smallestxblock << endl;
    }
  }
}
int main(){
