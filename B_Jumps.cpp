...
void solve(){
  ...
  while(l <= r){
    ...
    ll distance = (mid * (mid + 1)) / 2;
    // 1 + 2 + ... + mid;
    // we can make any of them as -1
    // so we can achive any distance like distance - 2, - 3, - 4
    // can't achive -1, as smallest is from 1 to -1, having -2 diff
