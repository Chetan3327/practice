...
void solve(){
  ...
  while(!pq.empty()){
    ...
    for(auto itr: adj[city]){
      ...
      ll newbike = bike;
      if(s[adjnode] < s[bike]) newbike = adjnode;

      if(dist[adjnode][newbike] < dist[city][bike] + wt * s[bike]) continue;
      dist[adjnode][newbike] = dist[city][bike] + wt * s[bike];
      pq.push({dist[adjnode][newbike], {adjnode, newbike}});
    }
  }
