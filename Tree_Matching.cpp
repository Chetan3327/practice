...
int dp[200001][2];
ll dfs(ll node, ll par, ll include, vvll &adj){
  if(dp[node][include] != -1) return dp[node][include];

  if(include){
    // include node, so we have to pair one more neighbour with it;

    ll sum = 0;
    for(auto itr: adj[node]){
      if(itr != par){
        sum += dfs(itr, node, 1, adj);
      }
    }
    ...
    for(auto itr: adj[node]){
      if(itr != par){
        // here we are using pair (node - itr), so we can include other neighbours.
        ans = max(ans, 1 + dfs(itr, node, 0, adj) + sum - dfs(itr, node, 1, adj));
      }
    }

    return dp[node][include] = ans;
  }else{
    // don't include node, so include all neighbours;

    ll ans = 0;
    for(auto itr: adj[node]){
      if(itr != par){
        ans += dfs(itr, node, 1, adj);
      }
    }
    return dp[node][include] = ans;
  }
...
void solve(){
  ...
  ll ans = max(dfs(1, -1, 0, adj), dfs(1, -1, 1, adj));
  print(ans);
}
int main(){
  fastio
